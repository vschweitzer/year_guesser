    ...
    for collection_index in input_dict:
        ...
        for item_index in input_dict[collection_index]:
            ...
            for page_index in input_dict[collection_index][item_index]:
                ...
                add_item_and_related(
                    item,
                    page,
                    input_dict[collection_index][item_index][page_index],
                    [collection],
                    skip=skip,
                )
...
def add_item_and_related(
    ...
    print("_prov:" + str(provider))
    item_id = hasher((item + "/" + str(page)).encode("utf-8")).hexdigest()
    # print(item_id)
    to_save.append(
        add_item_if_not_exists(
            item_id, item, page, item_dict, provider=provider, skip=skip
        )
    )
    ...
    for style in item_dict["cite_this"]:
        to_save.append(
            add_citation_if_not_exists(item_id, style, item_dict["cite_this"][style])
        )
...
def add_item_if_not_exists(
    ...
    print("_prov:" + str(provider))
    _provider = Provider.objects.get(pk=provider)
...
def add_image_if_not_exists(
    ...
    _license = License.objects.get(pk=license)
    ...
    if not image_exists:
        ...
        i = Image(item=item, image_id=image_id, license=_license)
...
def run():
    ...
    for prov in Provider.objects.all():
        print(prov.pk)
        p = Provider.objects.get(pk=prov.pk)
        p = Provider.objects.get(pk="loc")
    print("loading")
