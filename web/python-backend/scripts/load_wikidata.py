...
def get_apa_citation(result: dict):
    ...
    date: str = apa_date(parse_date(result["idate"]["value"]), int(result["iprecision"]["value"]))
    title: str
    if "title" in result:
...
def check_and_rescale_image(path: str):
    ...
    if max(image.size) > max_dimension + 1: # avoid multiple rescales
        ...
        new_dims: tuple = (round(image.width * scale_factor), round(image.height * scale_factor))
...
def download_image(url: str, image_path: str, contact_info: str, session: typing.Optional[requests.Session] = None):
...
def run(*args):
    ...
    if not args:
        print("Contact info required. Please provide it as the first argument (--script-args <info>)")
        exit(1)
    ...
    while not end:
        ...
        for result in results["results"]["bindings"]:
            ...
            item_dict = {
                ...
                "label": result["itemLabel"]["value"] if "itemLabel" in result else None,
                "alt_label": result["itemAltLabel"]["value"] if "itemAltLabel" in result else None,
                "description": result["itemDescription"]["value"] if "itemDescription" in result else None,
            }         

            citations: typing.Dict[str, str] = {
                "apa": get_apa_citation(result)
            }

            image_file_name: str = get_filename_from_url(image_url)
            image_file_ending: str = image_file_name.split(".")[-1] if "." in image_file_name else ""
            image_pk: str = date_guesser_operations.get_item_pk(item_id=item_id, image_url=image_url)

            print(image_pk)
            try:
                download_image(image_url, os.path.join(image_dir, image_pk + "." + image_file_ending), contact_info=contact_info, session=session)
            except Exception as ex:
                print(ex)
                continue

            date_guesser_operations.add_item_and_related(item_id=item_id, item_dict=item_dict, provider_pk=provider, citations=citations, image_url=image_url, image_base_dir=image_dir)
            item_count += 1
